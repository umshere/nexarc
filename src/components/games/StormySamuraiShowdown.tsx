...
const StormySamuraiShowdown: React.FC<StormySamuraiProps> = ({ onClose }) => {
  ...
  const handleJump = useCallback(() => {
    ...
    // Allow jump if the samurai is on or below ground (y >= 585)
  ...
  const draw = () => {
    ...
    if (gameStatus === 'start') {
      ...
      ctx.fillStyle = '#fff';
      ctx.textAlign = 'right';
      ctx.fillText('Press Space to Jump', canvas.width - 20, 50);
    }
  ...
  const startGame = () => {
    ...
    // Set samurai on ground at y=585
    samuraiRef.current = { x: 80, y: 585, width: 60, height: 75 };
    samuraiVelocityRef.current = 0;
